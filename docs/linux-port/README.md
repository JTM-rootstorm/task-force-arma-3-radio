...
The experimental Linux target now packages the full TFAR command processor, audio/radio runtime, bridge server, Clunk, DSP filters, and SQLite into a native Linux TeamSpeak plugin. The output file is `TFAR_win64_x64.so` so TeamSpeak routes plugin commands through the same namespace as existing Windows clients.
