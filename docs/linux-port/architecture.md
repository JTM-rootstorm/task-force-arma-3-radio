...
Build artifact: `TFAR_win64_linux_amd64.so`
...
The Linux build deliberately uses the Windows x64 TFAR base name plus TeamSpeak's Linux platform suffix. TeamSpeak strips `_linux_amd64` for the plugin-command namespace, so the registered command name should be `TFAR_win64`. Installing it as `TFAR_linux_x64.so` isolates VOLUME/TANGENT commands from Windows users; installing it as `TFAR_win64_x64.so` can make TeamSpeak assign a GUID namespace that stock Windows TFAR ignores.
