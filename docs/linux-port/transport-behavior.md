...
The Linux `TFAR_linux_amd64.so` artifact now links the full TFAR `CommandProcessor`, TeamSpeak callback surface, Clunk audio stack, DSP filters, SQLite, and the loopback `LinuxBridgeServer`.
...
- Loading `build/tfar-linux/TFAR_linux_amd64.so` with `ctypes.CDLL` succeeds.
- `ts3plugin_apiVersion()` returns `26` for the Linux TeamSpeak 3.6.2 target.
- `ldd -r build/tfar-linux/TFAR_linux_amd64.so` reports no unresolved plugin-owned symbols.
