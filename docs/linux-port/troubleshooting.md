...
- Confirm `TFAR_linux_amd64.so` is in the TeamSpeak plugin directory.
- Confirm stale alternate plugin files such as `TFAR_linux_x64.so`, `TFAR_win64_x64.so`, and `TFAR_win64_linux_amd64.so` are removed or disabled. Only `TFAR_linux_amd64.so` is supported.
